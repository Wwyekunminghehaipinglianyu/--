...
public class MenuController {
    ...
    public String getMenu(HttpServletRequest request) {
		...
		if (1 == level) {
			...
			array.add(getJsonObject("typeInfo", "商品类型信息", "layui-icon-table", "typeInfo"));
			array.add(getJsonObject("goodsInfo", "商品详情信息", "layui-icon-table", "goodsInfo"));
		...
		if (2 == level) {
			JSONArray array = new JSONArray();
			array.add(getJsonObject("accountUser2Info", "个人信息", "layui-icon-user", "accountUser2Info"));
			tableObj.putOpt("list", array);
		}
    ...
    public Result getTotle() {
		...
		resultMap.put("user2Info", user2InfoService.findAll().size());
