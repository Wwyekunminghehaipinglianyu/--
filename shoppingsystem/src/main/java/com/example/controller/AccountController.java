...
public class AccountController {
    ...
    public Result login(@RequestBody Account account, HttpServletRequest request) {
		...
		if (2 == level) {
			login = user2InfoService.login(account.getName(), account.getPassword());
		}
    ...
    public Result register(@RequestBody Account account) {
		...
		if (2 == level) {
			User2Info info = new User2Info();
			BeanUtils.copyProperties(account, info);
			login = user2InfoService.add(info);
		}
    ...
    public Result getAccountInfo(HttpServletRequest request) {
		...
		if (2 == level) {
			return Result.success(user2InfoService.findById(account.getId()));
		}
    ...
    */
    ...
    public Result getAuthorityInfo(HttpServletRequest request) {
    ...
    public Result getPermission(@PathVariable Integer modelId, HttpServletRequest request) {
        List<AuthorityInfo> authorityInfoList = JSONUtil.toList(JSONUtil.parseArray(authorityStr), AuthorityInfo.class);
        Account user = (Account) request.getSession().getAttribute("user");
        if (user == null) {
            return Result.success(new ArrayList<>());
    ...
    public Result updatePassword(@RequestBody Account info, HttpServletRequest request) {
		...
		if (2 == level) {
			...
			BeanUtils.copyProperties(info, user2Info);
    ...
    public Result resetPassword(@RequestBody Account account) {
		...
		if (2 == level) {
			User2Info info = user2InfoService.findByUserName(account.getName());
			if (info == null) {
				return Result.error(ResultCode.USER_NOT_EXIST_ERROR.code, ResultCode.USER_NOT_EXIST_ERROR.msg);
			}
			info.setPassword(SecureUtil.md5("123456"));
			user2InfoService.update(info);
		}
