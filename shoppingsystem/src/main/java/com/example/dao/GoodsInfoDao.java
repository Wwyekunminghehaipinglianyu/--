...
public interface GoodsInfoDao extends Mapper<GoodsInfo> {
    @Select("select * from goods_info left join type_info on goods_info.typeId = type_info.id where goods_info.typeId = #{typeId} and goods_info.state = '上架' limit 8")
    List<GoodsInfo> findByType(@Param("typeId") Integer typeId);

    @Select("select * from goods_info where recommend = '是' and state = '上架' order by id desc")
    List<GoodsInfo> findRecommendGoods();

    @Select("select * from goods_info where state = '上架'  order by sales desc")
